...
from derisk.agent import ProfileConfig, Agent, AgentMessage, ActionOutput, ConversableAgent
...
from derisk_ext.agent.agents.open_rca.actions.rca_planning_action import TaskPlan

logger = logging.getLogger(__name__)


class RcaReactPlanChatManager(ManagerAgent):
    ...
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "Rca Manager",
            category="agent",
            key="derisk_agent_team_rca_profile_name",
        ),
        role=DynConfig(
            "RcaReActManager",
            category="agent",
            key="derisk_agent_team_rca_profile_role",
        ),
        goal=DynConfig(
            ...
            category="agent",
            key="derisk_agent_team_rca_profile_goal",
        ),
        desc=DynConfig(
            "Advance the task reasoning_engine generated by the planning agent.",
            category="agent",
            key="derisk_agent_team_rca_profile_desc",
        ),
    )
        ...
        try:
            ...
            for i in range(self.max_round):
                ...
                if plan_message.action_report.is_exe_success:
                    ...
                    task_params: List[TaskPlan] = [TaskPlan(**item) for item in task_list]
                else:
                    continue
                ...
                for result in results:
                    ...
                    if result.name == reporter.name:
                        ...
                        self.last_rounds = result.rounds + 1
