...
from derisk_ext.agents.expand.action.react_action import ReActAction
...
class ReActAgent(ConversableAgent):
    end_action_name: str = DynConfig(
        "terminate",
        category="agent",
        key="derisk_agent_expand_plugin_assistant_agent_end_action_name",
    )
    max_steps: int = DynConfig(
        10,
        category="agent",
        key="derisk_agent_expand_plugin_assistant_agent_max_steps",
    )
    profile: ProfileConfig = ProfileConfig(
        ...
        constraints=DynConfig(
            ...
            category="agent",
            key="derisk_agent_expand_plugin_assistant_agent_constraints",
        ),
    ...
    async def generate_reply(
        ...
        **kwargs,
        ...
        try:
            ...
            reply_message: AgentMessage = await self.init_reply_message(
            ...
            reply_message.prompt = system_prompt
            ...
            while not done and max_steps > 0:
                ...
                try:
                    # 1. thinking
                    llm_thinking, llm_reply, model_name = await self.thinking(
