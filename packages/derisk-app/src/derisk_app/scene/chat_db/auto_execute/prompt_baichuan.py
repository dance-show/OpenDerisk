...
from derisk.core.interface.prompt import PromptTemplate
from derisk_app.scene import ChatScene
from derisk_app.scene.chat_db.auto_execute.out_parser import DbChatOutputParser
...
注意：
1. 只能使用表结构信息中提供的表来生成 sql，如果无法根据提供的表结构中生成 sql ，\
请说：“提供的表结构信息不足以生成 sql 查询。” 禁止随意捏造信息。
2. 不要查询不存在的列，注意哪一列位于哪张表中。
3. 使用 json 格式回答，确保你的回答是必须是正确的 json 格式，\
并且能被 python 语言的 `json.loads` 库解析, 格式如下：
{response}
"""
