...
import { apiInterceptors, getMPCListQuery, mcpToolConnect, mcpToolList, mcpToolRun } from '@/client/api';
...
const MpcDetail: React.FC = () => {
  ...
  const [isConnect, setIsConnect] = useState<any>(false);
  ...
  const { data: mcpData, loading: mcpQueryLoading } = useRequest(
    async (params, other): Promise<any> => {
  ...
  const { loading: connectLoading, run: runMcpToolConnect } = useRequest(
    async (
      params = {
        name: queryParams.name,
      },
    ): Promise<any> => {
      return await apiInterceptors(mcpToolConnect(params));
    },
    {
      manual: true,
      onSuccess: data => {
      ...
      <div className='page-body p-4 md:p-6 h-[90vh] overflow-auto'>
        ...
        <div className='mb-3 flex flex-nowrap'>
          ...
          {/* <div className='flex flex-nowrap ml-3'>
            {isConnect && <Button size='large' icon={<ApiOutlined />} onClick={handleDisconnect} />}
            <Button type='primary' size='large' onClick={onGoRun}>
              {t('run')}
            </Button>
          </div> */}
        </div>
            ...
            >
              <div className='flex'>
                <div className='w-full'>
                  <Form className='font-[400]' onFinish={goRun} style={{ width: '100%' }} form={form}>
