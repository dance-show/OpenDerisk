...
function ChatInput(prosp: propsT) {
  const { bodyClassName, minRows, maxRows = undefined, sendClassName, cneterBtn = true, isGoPath = false } = prosp;
  ...
  const onSubmit = async () => {
    ...
    if (isGoPath) {
      ...
      if (initRes) {
        const [, res] = await apiInterceptors(newDialogue({ chat_mode: initRes?.main_app, model_name: initRes?.main_chat_mode }));
        if (res) {
